...
const CRCDetails = () => {
    ...
    const {
      ...
      const [crcdData,setCrcdData]=useState({
        icu:"",ot:"",specialist:"",med:"",total:""
    })
    const handleInputChange = (e) => {
        const { name, value } = e.target;
        setCrcdData((prevData) => ({
          ...prevData,
          [name]: value,
        }));
      };
      const calculateTotal = () => {
        const total = Object.values(crcdData).reduce((acc, val) => (acc + parseFloat(val || 0)), 0);
  ...
  return (
    ...
    <GptIntegration crcdData={crcdData}/>
    </div>
  )
}
